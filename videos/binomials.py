...
class APlusBSquared(Scene):
    def construct(self):
        ...
        # Draw square
        big_square = (
            Square(side_length=a + b, fill_opacity=opacity, fill_color=GREEN,)
            .to_edge(DOWN)
            .shift(0.5 * UP)
        )
        ...
        for (corner, width, height, color, text) in squares_data:
            square = Rectangle(
                width=width, height=height, fill_opacity=opacity, fill_color=color,
        ...
        for i in range(0, len(terms)):
            ...
            self.play(Write(link), Transform(areas[i], terms[i].next_to(link, RIGHT)))
            previous = areas[i]
        self.wait()

        # Collect like terms
        two_ab = (
            TexMobject("+", "2ab").scale(1.5).next_to(VGroup(*groups["a^2"]), RIGHT)
        )
...
class ASquaredMinusBSquared(Scene):
    def construct(self):
        ...
        # Big square
        big_square = (
            Square(side_length=a, fill_opacity=opacity, fill_color=BLUE,)
            .to_edge(DOWN)
            .shift(0.5 * UP)
        )
        a_squared = big_square.write("a^2")
        self.play(FadeIn(big_square), Write(a_squared))
        self.wait()

        # Small square
        small_square = Square(side_length=b, fill_opacity=1, fill_color=RED,).move_to(
            big_square.get_corner(DR), DR
        )
        b_squared = small_square.write("b^2")
        self.play(FadeIn(small_square), Write(b_squared))
        self.wait()
        ...
        self.play(
            Transform(a_squared, lhs1), Transform(b_squared, lhs2),
        )
        ...
        # Split shape
        rect = Rectangle(
            width=a, height=a - b, fill_color=GREEN, fill_opacity=opacity,
        ).move_to(big_square.get_corner(UL), UL)
        rect2 = Rectangle(
            width=a - b, height=b, fill_color=GREEN, fill_opacity=opacity,
        ).move_to(big_square.get_corner(DL), DL)
        self.play(
            FadeIn(VGroup(rect, rect2)), FadeOut(shape),
        )
        self.wait()

        # Reorganise pieces
        self.play(
            ApplyMethod(VGroup(rect2, b_label).shift, b * UR), FadeOut(b_label),
        )
        ...
        self.play(
            FadeIn(eq), Transform(aplusb.copy(), rhs1),
        )
        self.wait()
        self.play(Transform(aminusb.copy(), rhs2),)
        self.wait(5)
