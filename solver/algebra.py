...
def exercise(expr, callback_ex, callback_sol, **options):
    substitutions = [
        (s.symbols(t), s.UnevaluatedExpr(v))
        for (t, v) in options.items()
        if t not in ["l"]
    ]
    exercise = (
...
def _mult(join_ex, join_sol, terms, options, cbk=latex, div=False):
    ...
    solution = _simplify(f"({join_sol.join([str(t) for t in terms])})").subs(
        substitutions
    )
...
def power(expr, power, **substitutions):
    substitutions = [
        (s.symbols(t), s.UnevaluatedExpr(v)) for (t, v) in substitutions.items()
    ]
